...
import random
import json  # NEW: used to parse LLM JSON output
...
def init_db():
    ...
    # We'll add 'summary_text' (TEXT), 'recommendation' (TEXT), 'confidence' (REAL) columns.
    cur.execute("PRAGMA table_info(interview_reports)")
    cols = [row["name"] for row in cur.fetchall()]
    ...
    if not rows:
        ...
        for r in rows:
            ...
            with col_summary:
                ...
                # BUTTON behavior:
                # - If summary exists: show "View TL;DR" and "🔄 Regenerate Summary"
                # - If not: show "⚡ Quick Summary"
                if existing_summary:
                    # Button to view the stored TL;DR (will open expander with full saved content)
                    if st.button("Quick Summary", key=f"view_{r['id']}"):
                        # Show the saved human-readable summary in an expander
                    ...
                    # Regenerate button (separate key)
                    ...
                    # No summary yet -> Quick Summary button
                    if st.button("⚡ Quick Summary", key=f"gen_{r['id']}"):
                        with st.spinner("Generating TL;DR & hiring recommendation..."):
                            ...
                            # display the created summary immediately inside an expander
